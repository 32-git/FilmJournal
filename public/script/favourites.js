...
document.addEventListener("DOMContentLoaded", () => {
  ...
  getFavourites(username, movieWrapper, seriesWrapper);

  document.getElementById("sort-button").onclick = allowDrag;
});

async function getFavourites(user, wrapper1, wrapper2) {
  const data = await fetch("/get-favourites", {
    method: "POST",
    headers: {
